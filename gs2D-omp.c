/* 2D Gauss-Seidel smoothing to solve -u'' = f */
...
#pragma omp parallel for default(none) shared(n,u,f) private(i)
...
#pragma omp parallel for default(none) shared(n, u,f, h2) private(i,j)
    for (i = 1; i < n+1; ++i){
      for (j = (i % 2)+1 ; j < n+1; j += 2){
	u[j+(n+2)*i] = (h2 * f[j+(n+2)*i] + u[j+(n+2)*i+1] + u[j+(n+2)*i-1]+u[j+(n+2)*(i-1)]+u[j+(n+2)*(i+1)]) / 4.0f;
      }
    }		 
...
/* Compute residual */
    res = 0.0;
#pragma omp parallel for default(none) shared(u, f, n, h2) private(i,j,tmp) reduction(+:res)
      for (i = 1; i < n+1; ++i){
	for (j = 1; j < n+1; ++j){
	  tmp = (double)pow((f[j+(n+2)*i]- (4.0f* u[j+(n+2)*i] - u[j+(n+2)*i+1] - u[j+(n+2)*i-1]-u[j+(n+2)*(i-1)]-u[j+(n+2)*(i+1)])/h2),2);
	  res += tmp;
	}
      }
    p = p + 1;
